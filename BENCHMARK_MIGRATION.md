...
The following benchmarks with Timely Dataflow and Differential Dataflow dependencies have been moved from `bigweaver-agent-canary-hydro-zeta` to this repository:

- **`fan_out.rs`** - Fan-out dataflow patterns with Timely comparisons
- **`fan_in.rs`** - Fan-in dataflow patterns with Timely comparisons
- **`fork_join.rs`** - Fork-join patterns with Timely comparisons
- **`reachability.rs`** - Graph reachability with Differential Dataflow comparisons
...
Additionally, the following benchmarks were also moved to maintain the complete benchmark suite:
...
## Dependencies Isolated in hydro-deps Repository

These dependencies are now maintained only in `bigweaver-agent-canary-zeta-hydro-deps/benches/Cargo.toml`:
...
- Developers working on the main repository benefit from faster build times
- Performance comparison functionality is retained through this separate benchmark repository
...
Performance comparison functionality is fully retained through this separate benchmark repository. The migration maintains the ability to compare Hydro implementations against Timely Dataflow and Differential Dataflow baselines.

### Step 1: Run Benchmarks in hydro-deps Repository

Run benchmarks that use Timely and Differential Dataflow:
...
Results will be saved in `target/criterion/` with detailed HTML reports.
