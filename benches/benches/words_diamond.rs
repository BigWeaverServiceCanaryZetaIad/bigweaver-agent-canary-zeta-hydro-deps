...
use nameof::name_of;
use timely::dataflow::operators::{Inspect, Filter, Map, ToStream, Concat};
...
fn timely_diamond(c: &mut Criterion) {
    let _ = *WORDS;

    c.bench_function(name_of!(timely_diamond), |b| {
        b.iter(|| {
            ...
            timely::example(|scope| {
                ...
                let stream = words_vec.to_stream(scope);
                
                // First branch: flat_map
                let branch1 = stream.map(|s| vec![format!("hi {}", s), format!("bye {}", s)])
                    .flat_map(|v| v);
                
                // Second branch: filter
                let branch2 = stream.filter(|s| 0 == s.len() % 5);
                
                // Concatenate the two branches and count
                branch1.concat(&branch2)
                    .inspect(move |s| {
