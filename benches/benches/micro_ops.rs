...
use timely::dataflow::operators::{Inspect, Map, Filter, ToStream};
...
fn timely_ops(c: &mut Criterion) {
    ...
    c.bench_function("micro/ops/timely/identity", |b| {
        const NUM_INTS: usize = 10_000;
        let dist = Uniform::new(0, 100);
        let data: Vec<usize> = (0..NUM_INTS).map(|_| dist.sample(&mut rng)).collect();
    ...
    c.bench_function("micro/ops/timely/map", |b| {
        const NUM_INTS: usize = 10_000;
        let dist = Uniform::new(0, 100);
        let data: Vec<usize> = (0..NUM_INTS).map(|_| dist.sample(&mut rng)).collect();
    ...
    c.bench_function("micro/ops/timely/flat_map", |b| {
        const NUM_INTS: usize = 10_000;
        let dist = Uniform::new(0, 100);
        let data: Vec<usize> = (0..NUM_INTS).map(|_| dist.sample(&mut rng)).collect();
    ...
    c.bench_function("micro/ops/timely/filter", |b| {
        const NUM_INTS: usize = 10_000;
        let dist = Uniform::new(0, 100);
        let data: Vec<usize> = (0..NUM_INTS).map(|_| dist.sample(&mut rng)).collect();
...
criterion_group!(micro_ops, ops, timely_ops);
