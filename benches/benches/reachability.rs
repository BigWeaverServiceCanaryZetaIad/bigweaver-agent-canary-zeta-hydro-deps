        ...
        b.iter(move || {
            timely::execute_directly(move |worker| {
                let probe = worker.dataflow::<u32, _, _>(|scope| {
                    ...
                    let roots = scope.new_collection_from(1_usize..=1).1;
...
// Hydroflow benchmark functions removed - they remain in the main repository for comparison

fn benchmark_sol(c: &mut Criterion) {
    let edges = &*EDGES;
    let reachable = &*REACHABLE;

    c.bench_function("reachability/sol", |b| {
        b.iter(|| {
            let mut seen = HashSet::new();
            let mut new = vec![1];
            while !new.is_empty() {
